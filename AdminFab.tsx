...
import { Lock, X, ShieldCheck, Loader2, Globe } from 'lucide-react';
...
const AdminFab: React.FC = () => {
  ...
  const publishToGithub = async () => {
    ...
    if (!token || !repo) { 
      setStatus('error'); 
      setStatusMsg('Configura GitHub primero.'); 
      return; 
    }
...
export const REVIEWS = ${JSON.stringify(REVIEWS, null, 2)};`;
      ...
      if (putRes.ok) {
        ...
        setStatus('success'); 
        setStatusMsg(`¡Web v${nextVer} publicada!`);
    ...
    <div className="fixed bottom-8 right-8 z-[150] no-print flex flex-col items-end gap-4">
      {status !== 'idle' && (
        <div className={`p-4 rounded border-l-4 w-[300px] bg-zinc-900 shadow-2xl ${status === 'loading' ? 'border-amber-500 text-amber-500' : status === 'success' ? 'border-green-500 text-green-400' : 'border-red-500 text-red-400'}`}>
          <p className="text-[10px] font-bold uppercase tracking-widest">{statusMsg}</p>
        </div>
      )}
